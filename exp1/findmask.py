...
def GenRandomData(batch_size=64):
    x = torch.rand(batch_size, 1, 20, 20) * 0.5
    ...
    for i in range(batch_size):
        ...
        if pos_prob < 0.5:
            ...
            x[i, 0, y_sh: 4 + y_sh, x_sh] = 1.0
            x[i, 0, y_sh, x_sh : 4 + x_sh] = 1.0
            y[i, 0] = 1.0
...
optimizer = torch.optim.Adam(model.parameters(), weight_decay=0.0, lr=0.001)
